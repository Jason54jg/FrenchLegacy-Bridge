...
async function updateRosterEmbed(guild) {
    ...
    guildInfo.ranks.forEach(rank => {
        ...
        guildInfo.memberInfo.forEach(member => {
            if (member.rank != rank.name) { return; }
        ...
        for (let i = 0; i < membersValues.length; i++) {
            fields.push({ name: `${i == 0 ? `${rank.name} ${rank.tag != null ? `[${rank.tag}]` : ''} » ${rankSize}` : '\u200b'}`, value: `\u200b${membersValues[i]}` });
        }
    })
    ...
    const embed = new EmbedBuilder()
        ...
        .setDescription(`Nombre de membres de la guilde : **${guildInfo.memberInfo.length}/125**`)
        .addFields(fields)
    return embed;
}