...
module.exports = {
  ...
  execute: async (interaction, client) => {
    try {
      ...
      const id = interaction.options._hoistedOptions[1]
        .value(await interaction.guild.members.fetch(id))
        .roles.add(
          interaction.guild.roles.cache.get(config.discord.roles.linkedRole)
        );
      ...
      const successfullyLinked = new EmbedBuilder()
        ...
        .setDescription(
          `\`${username}\` a été lié avec succès à \`${
