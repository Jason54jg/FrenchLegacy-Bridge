        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId.includes("giveawayWinInfo")) {
                return await interaction.reply({
                    content: "Pour pouvoir recevoir ta récompense tu devras faire un ticket -> <#999022746619084961> et ping l'host du giveaway",
                    ephemeral: true
                })
            }
        }
...
async function manageGiveawayButtons(interaction) {
    ...
    if (giveaway == null) {
        return await interaction.reply({
            content: "Vous ne pouvez plus intéragir avec ce giveaway car il est terminé !",
            ephemeral: true,
        });
    }
    ...
    if (id.length > 2) {
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(`Total: ${participants.length} participants \n${description}`)
        ...
        let newActionRowEmbeds = interaction.message.components.map(oldActionRow => {
            ...
            updatedActionRow.addComponents(oldActionRow.components.map(buttonComponent => {
                ...
                if (buttonComponent.customId.split('_').length == 2) {
                    newButton.setLabel(`${participants.length + 1} Participant${participants.length != 1 ? "s" : ""}`)
                }
                return newButton
            }));
