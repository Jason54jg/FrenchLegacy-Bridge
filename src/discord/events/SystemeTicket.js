...
module.exports = {
  ...
  execute: async (interaction, client) => {
    ...
    if (interaction.customId.includes("ticket-close")) {
      ...
      interaction.customId.includes("ticket-delete") &&
        ...
        .then((c) => {
          c.send({
            ...
            embeds: [
              {
                description:
                  "Bonjour , Nous vous remercions pour avoir prit contact avec nous. Un membre du staff vous aidera dans les plus brefs délais. En attendant, veuillez nous décrire la raison de ce ticket le plus précisemment possible.",
                footer: {
                  text: "FrenchLegacy",
                  iconURL:
            ...
            components: [
              new ActionRowBuilder().addComponents(buttonCloseTicket),
            ],
          });
          interaction.reply({
        ...
        .then((c) => {
          c.send({
            ...
            embeds: [
              {
                description:
                  "Veuillez nous indiquer dans quelle catégorie vous voulez faire des carry",
                footer: {
                  text: "BlackPast",
                  iconURL:
                    "https://media.discordapp.net/attachments/1073744026454466600/1076983462403264642/icon_FL_finale.png",
      ...
      if (interaction.member.roles.cache.has("999026248590315661")) {
        ...
        interaction.reply({
          content: `Votre rôle VIP vous a été retiré avec succès.`,
          ephemeral: true,
        });
        Logger.discordMessage(
          `[Grade] VIP a été retiré à ${interaction.user.username}`
        );
      } else {
        interaction.member.roles.add("999026248590315661");
        interaction.reply({
          content: `Votre rôle VIP vous a été attribué avec succès.`,
          ephemeral: true,
        });
        Logger.discordMessage(
          `[Grade] VIP a été donné à ${interaction.user.username}`
        );
      }
    } else if (interaction.customId == "roles_vip+") {
      ...
      if (interaction.member.roles.cache.has("999026463569367142")) {
        ...
        interaction.reply({
          content: `Votre rôle MVP vous a été retiré avec succès.`,
          ephemeral: true,
        });
        Logger.discordMessage(
          `[Grade] MVP a été retiré à ${interaction.user.username}`
        );
      } else {
        interaction.member.roles.add("999026463569367142");
        interaction.reply({
          content: `Votre rôle MVP vous a été attribué avec succès.`,
          ephemeral: true,
        });
        Logger.discordMessage(
          `[Grade] MVP a été donné à ${interaction.user.username}`
        );
      }
    } else if (interaction.customId == "roles_mvp+") {
...
async function manageModalInteraction(interaction, client) {
  if (interaction.customId.includes("claim-end")) {
    ...
    await firstMessage.edit({
      content: `${firstMessage.content.split("|")[0]}|${
        firstMessage.content.split("|")[1]
      }| ${nbCarryDone + carryAmount}/${nbCarryTotal} carry fait -> ${
        (carryToDo - carryAmount) * pointsPerCarry
      } point${(carryToDo - carryAmount) * pointsPerCarry == 1 ? "" : `s`}${
        carryToDo - carryAmount == 1 ? "" : ` (${pointsPerCarry}/carry)`
      }`,
      components: [
        new ActionRowBuilder().addComponents(
          buttonCloseTicket,
  ...
  interaction.guild.channels
    ...
    .then((c) => {
      c.send({
        content: `${role} | ${
          interaction.user
        } IGN: ${ign} | 0/${carryAmount} carry fait -> ${
          points * carryAmount
        } point${points * carryAmount == 1 ? "" : `s`}${
          carryAmount == 1 ? "" : `(${points}/carry)`
        }`,
        embeds: [
          {
