...
class PersonalBestCommand extends minecraftCommand {
    constructor(minecraft) {
        ...
        this.description = "Renvoie le temps le plus rapide (s+) de n'importe quel donjon";
        this.options = [
            {
                name: "username",
            ...
            {
                name: "Floor",
                description: "Étage de donjons (M7, F7, ect)",
                required: false,
            },
        ];
    }
        ...
        try {
            ...
            switch (floor_type) {
                ...
                case "e":
                    ...
                    break;
                default:
                    this.send("/gc Utilisation invalide : !pb [utilisateur] [étage (m7/f4/etc)] [rang (S+, S, any)]");
                    break;
            }
            // eslint-disable-next-line no-throw-literal
            if (requested_floor === null) throw `${username} n'a jamais obtenu de ${rank} sur ${floor} auparavant.`;
            ...
            if (time === 0) {
                this.send(`/gc ${username} n'a pas de PB en ${floor} ${rank}`);
            } else {
                this.send(`/gc Le PB de ${username} en ${floor} avec le rang ${rank} est de ${millisToMinutesAndSeconds(time)}`);
            }
        } catch (error) {
            console.log(error);
