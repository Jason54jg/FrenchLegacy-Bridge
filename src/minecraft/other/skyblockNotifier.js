...
async function checkForIncidents() {
  try {
    const status = await parser.parseURL(
      "https://status.hypixel.net/history.rss"
    );
    for (const data of status.items) {
      const currentStatus = JSON.parse(
        fs.readFileSync("data/skyblockNotifer.json")
      ).skyblockStatus;
      const contents = JSON.stringify(data.contentSnippet)
        .trim()
        .replaceAll('"', "")
        .split("\\n");
      for (const content of contents) {
        ...
        if (
          currentStatus.includes(`${data.title} | ${content} | ${date}`) ===
          true
        )
          continue;
...
async function checkForSkyblockUpdates() {
  try {
    ...
    for (const data of feed) {
      const currentUpdates = JSON.parse(
        fs.readFileSync("data/skyblockNotifer.json")
      ).skyblockUpdates;
      if (currentUpdates.includes(`${data.title} | ${data.link}`) === true)
        continue;
