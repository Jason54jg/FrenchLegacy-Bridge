...
async function guild_list(guild) {
    ...
    const rep = await requete.get_page(`${hypixel_api}/guild?key=${api_key}&name=${guild}`);
    if (! rep.success) {
    ...
    ranks.sort((a, b) => { return b.priority - a.priority });
    ...
    for (let i = 0; i < guilde.members.length; i++) {
        ...
        const player = await requete.get_page(`${hypixel_api}/status?key=${api_key}&uuid=${member.uuid}`);
        player.success ? playerInfo.online = player.session.online : playerInfo.online = false;

        // Récuperer le pseudo du joueur
        const response = await requete.get_page(Mojang + member.uuid);
        response != null ? playerInfo.name = response.name : playerInfo.name = '???';
