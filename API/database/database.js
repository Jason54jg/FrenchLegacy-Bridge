...
class DB {
    ...
    async getUserByScoreOrder(currentPage, userPerPage) {
        if((currentPage-1)*userPerPage == 0){
            return await this.clientDb.db("dev").collection("users").find({ score: { $ne: 0 } }).sort({ score: -1, _id: 1 }).limit(userPerPage).toArray();
        }
        return await this.clientDb.db("dev").collection("users").find({ score: { $ne: 0 } }).sort({ score: -1, _id: 1 }).skip((currentPage-1)*userPerPage).limit(userPerPage).toArray();
    }

    // Récupérer un utilisateur de la db
    async getUserById(discordId) {
        return await this.clientDb.db("dev").collection("users").findOne({ discordId: discordId });
    }
