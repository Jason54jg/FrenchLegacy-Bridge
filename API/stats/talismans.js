...
const {
  capitalize,
  decodeData,
} = require("../../src/contracts/helperFunctions.js");

module.exports = async (profile) => {
  if (profile.talisman_bag?.data) {
    const talismans = {
      talismanBagUpgrades:
        profile.accessory_bag_storage?.bag_upgrades_purchased || 0,
      ...
      tuningsSlots:
        profile.accessory_bag_storage.tuning?.highest_unlocked_slot || 0,
    ...
    for (const talisman of talisman_bag) {
      if (talisman.tag?.display.Name && talisman.tag?.ExtraAttributes) {
        ...
        if (talismans.talismans[getRarity(talisman.tag?.display.Lore)]) {
          talismans.talismans[getRarity(talisman.tag?.display.Lore)].push(
            new_talisman
          );
        } else {
          talismans.talismans[getRarity(talisman.tag?.display.Lore)] =
            new_talisman;
        }
      }
    }
